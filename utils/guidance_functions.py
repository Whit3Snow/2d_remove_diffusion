...
def fix_shapes_l2(orig_attns, edit_attns, indices, tau=fc.noop): # move 
    shapes = []
    for location in ["mid", "up", "down"]:
        for o in indices:
            deltas = []
...
def fix_shapes_l3(orig_attns, edit_attns, indices, tau=fc.noop): # resize 
    shapes = []
    for location in ["mid", "up", "down"]:
        for o in indices:
            deltas = []
...
def fix_shapes_l4(orig_attns, edit_attns, indices, tau=1): #remove
    shapes = []
    for location in ["mid", "up", "down"]:
        for o in indices:
            deltas = []
            for edit_attn_map_integrated, ori_attn_map_integrated in zip(edit_attns[location], orig_attns[location]):
                ...
                orig_shape = get_shape(orig)
                orig_shape[:,:,:] = 0
                delta = orig_shape - get_shape(edit)
...
def fix_appearances(orig_attns, ori_feats, edit_attns, edit_feats, indices):
    appearances = []
    # feats는 64 * 64 이기 때문에 'up'의 4096으로 할 수 있음.   
    for o in indices:
...
def edit_appearance_object(attn_storage, indices, shape_weight=1, **kwargs):
    origs, edits = get_attns(attn_storage)
    if len(indices) > 1: 
        obj_idx, other_idx = indices
        indices = torch.cat([obj_idx, other_idx])
    return 0
    # return shape_weight * fix_shapes_l4(origs, edits, obj_idx)
...
def move_object_by_shape_2(attn_storage, indices, tau=fc.noop, shape_weight=1, appearance_weight=0.5, position_weight=1, ori_feats=None, edit_feats=None, **kwargs):
    origs, edits = get_attns(attn_storage)
    ...
    if len(indices) > 1: 
        ...
        indices = torch.cat([obj_idx, other_idx])
    shape_term = shape_weight * fix_shapes_l1(origs, edits, other_idx)
    # appearance_term = appearance_weight * fix_appearances_by_feature(ori_feats, edit_feats, other_idx)
    # shape_term2 = shape_weight * fix_shapes_l4(origs, edits, obj_idx)
    appearance_term = appearance_weight * fix_appearances(origs, ori_feats, edits, edit_feats, other_idx)
    ...
    # return shape_term + appearance_term + shape_term2
    return shape_term + appearance_term + move_term
